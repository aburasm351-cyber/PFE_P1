    ...
    >
      ...
      <div className="flex items-center gap-2 flex-1 max-w-md">
        <div className="relative flex-1 hidden sm:block">
          <Search className={cn("absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground", isRtl ? "right-2.5" : "left-2.5")} />
          <Input
            placeholder={t("search")}
            className={cn(
              "h-7 w-full text-xs bg-muted/30 border-transparent focus:border-border focus:bg-background",
          ...
          {showResults && searchQuery.length >= 2 && (
            <>
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: 4 }}
                className={cn(
                  "absolute top-10 w-80 z-50 bg-card border border-border shadow-lg rounded-lg overflow-hidden",
      ...
      <div className={cn("flex items-center gap-1.5", isRtl && "flex-row-reverse")}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className={cn("h-7 gap-1.5 pl-1 pr-2", isRtl && "flex-row-reverse")}>
